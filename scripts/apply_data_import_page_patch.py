...
(function(){
  ...
  function ensureNavItem(){
    ...
    if(!document.querySelector('[data-view="data-update"]')){
      const btn = document.createElement('button');
      btn.className = 'nav-item';
      btn.dataset.view = 'data-update';
      btn.textContent = '数据导入';
      nav.appendChild(btn);
    }
  }

  function ensureImportPage(){
  ...
  async function loadImportBatches(){
    ...
    try{
      ...
      const cols = ['ID','类型','目录','市场','状态','文件数','成功','失败','记录数','开始时间','结束时间','信息'];
      table.innerHTML =
  ...
  async function loadImportFiles(){
    ...
    try{
      ...
      const cols = ['ID','批次ID','文件','市场','状态','导入行数','错误','开始时间','结束时间'];
      table.innerHTML =
        '<thead><tr>' + cols.map(c => `<th>${c}</th>`).join('') + '</tr></thead><tbody>' +
        rows.map(r => `
          <tr>
...
def main():
    ...
    js_script = '  /static/data_import_page.js?v=1script>'

    html = inject_once(
        html,
        js_script,
        "/static/data_import_page.js",
        "</body>"
    )

    INDEX.write_text(html, encoding="utf-8")

    print("OK: 已创建数据导入页面脚本 app/static/data_import_page.js")
    print(f"OK: 已备份 index.html -> {backup}")
    print("OK: 已注入 /static/data_import_page.js?v=1")
