(function(){
  ...
  async function load2568Annotations(){
    ...
    const cols=['代码','名称','指标状态','指标日期','指标重算时间','最新2560','2560计算时间','2560信号时间','人工建议','等级','A','B','D','亮点总结','A亮点','B亮点','D风险','总结','风险','MA25','MA60','价格','量能','趋势','回踩'];
    table.innerHTML='<thead><tr>'+cols.map(c=>`<th>${c}</th>`).join('')+'</tr></thead><tbody>'+rows.map(r=>`<tr><td>${cell(r.code)}</td><td>${cell(r.name)}</td><td><b>${cell(r.indicator_freshness_status)}</b></td><td>${cell(r.indicator_date)}</td><td>${cell(r.indicator_recalculated_at)}</td><td>${cell(r.latest_2560_status)}</td><td>${cell(r.latest_2560_run_time)}</td><td>${cell(r.latest_2560_signal_time)}</td><td><b>${cell(r.manual_action_label)}</b></td><td>${cell(r.highlight_level)}</td><td>${cell(r.a_count)}</td><td>${cell(r.b_count)}</td><td>${cell(r.d_count)}</td><td style="min-width:360px">${cell(r.highlight_summary)}</td><td>${cell(r.a_highlights)}</td><td>${cell(r.b_highlights)}</td><td>${cell(r.d_highlights)}</td><td>${cell(r.summary_label)}</td><td>${cell(r.risk_tags)}</td><td>${cell(r.ma25_status)}</td><td>${cell(r.ma60_status)}</td><td>${cell(r.price_status)}</td><td>${cell(r.volume_status)}</td><td>${cell(r.trend_status)}</td><td>${cell(r.pullback_status)}</td></tr>`).join('')+'</tbody>';
  }
