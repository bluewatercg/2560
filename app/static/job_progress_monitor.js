(function(){
  ...
  function ensureMonitorPanel(){
    ...
    panel.innerHTML = `
      <div class="panel-head split">
        <h3>当前执行状态</h3>
        <button id="jobProgressRefreshBtn">刷新进度</button>
      </div>
      <div id="jobProgressCards" class="cards"></div>
      <div style="margin:12px 0;height:12px;background:#e5e7eb;border-radius:999px;overflow:hidden">
        <div id="jobProgressBar" style="height:12px;width:0%;background:#2563eb"></div>
      </div>
      <pre id="jobProgressText" class="json-box">暂无执行任务</pre>
      <h3 style="margin-top:12px">实时日志</h3>
      <pre id="jobLiveLog" class="json-box" style="max-height:320px;overflow:auto">暂无日志</pre>
    `;
    const action = $('jobActionPanel');
    if(action && action.parentNode) action.parentNode.insertBefore(panel, action.nextSibling);
  ...
  function patchRunNowButton(){
    ...
    btn.onclick = async function(){
      ...
      $('jobActionResult') && ($('jobActionResult').textContent = JSON.stringify(data,null,2));
      startPolling();
    };
  }
  ...
  async function refreshProgress(){
    ...
    $('jobProgressCards').innerHTML = [
      ['任务ID', p.id], ['状态', p.status], ['市场', p.market || '-'], ['分片', p.shards || '-'],
      ['进度', `${p.done}/${p.total}`], ['百分比', `${percent}%`], ['成功', p.success_count || 0], ['失败', p.failed_count || 0],
      ['当前代码', p.current_code || '-'], ['已运行', p.elapsed_text || '-'], ['均耗时', p.avg_seconds_per_code ? `${p.avg_seconds_per_code}s/只` : '-'], ['预计剩余', p.eta_text || '-']
